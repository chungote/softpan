$PBExportHeader$w_abm_productos_old.srw
forward
global type w_abm_productos_old from w_abm_ancestor
end type
type tab_1 from tab within w_abm_productos_old
end type
...
global w_abm_productos_old w_abm_productos_old
...
long il_cod_producto,il_cod_tipo_producto,il_cod_grupo,il_cod_tipo_iva,il_cod_estante,il_cod_deposito,il_cod_medida,il_cod_pais,il_estado,il_tiene_receta,il_en_oferta
...
string is_notas,is_modo_preparacion
...
on w_abm_productos_old.destroy
call super::destroy
...
INSERT INTO RECETAS  
  ...
  VALUES ( :il_cod_receta,   
		...
		if sqlca.sqlcode=0 then
			messagebox("Aviso",upper("Registro guardado con exito"))
			commit using sqlca;
		else
		...
		INSERTANDO EL CODIGO DE INSUMO*/
  ...
  UPDATE PRODUCTOS  
	...
	WHERE  cod_producto=:il_cod_producto;
     ...
     SET   
         ...
         precio_venta = :id_precio_venta
	 WHERE /*cod_receta = :il_cod_receta */cod_producto = :il_cod_producto  ;

	
	if sqlca.sqlcode=0 then
			messagebox("Aviso","Registro actualizado con exito")
			commit using sqlca;
		else
	...
	if  tab_1.tabpage_recetas_productos.cdw_datos_detalle_02.update(True, False) = 1 then
		...
		rollback using sqlca;
			if is_mensaje_error <> "" then
				MessageBox("AVISO DE GRABACION",is_mensaje_error)	
				is_mensaje_error = ""
			end if
			tab_1.tabpage_recetas_productos.cdw_datos_detalle_02.RowsMove(1,1,delete!,cdw_datos,1,primary!)
...
type p_1 from w_abm_ancestor`p_1 within w_abm_productos_old
integer x = 2647
integer width = 763
end type

type pb_buscar from w_abm_ancestor`pb_buscar within w_abm_productos_old
integer x = 1568
integer y = 1844
...
type pb_cancelar from w_abm_ancestor`pb_cancelar within w_abm_productos_old
integer x = 2103
...
type pb_modificar from w_abm_ancestor`pb_modificar within w_abm_productos_old
integer x = 1632
integer taborder = 80
end type
...
type pb_borrar from w_abm_ancestor`pb_borrar within w_abm_productos_old
integer x = 1170
integer taborder = 160
end type

type pb_guardar from w_abm_ancestor`pb_guardar within w_abm_productos_old
integer x = 713
integer taborder = 40
end type

type pb_agregar from w_abm_ancestor`pb_agregar within w_abm_productos_old
integer x = 256
integer taborder = 150
end type

type ole_skin from w_abm_ancestor`ole_skin within w_abm_productos_old
end type

type st_4 from w_abm_ancestor`st_4 within w_abm_productos_old
integer x = 2624
integer y = 2008
end type

type st_3 from w_abm_ancestor`st_3 within w_abm_productos_old
integer x = 2258
integer y = 2008
end type

type st_2 from w_abm_ancestor`st_2 within w_abm_productos_old
integer x = 1897
integer y = 2008
end type

type st_1 from w_abm_ancestor`st_1 within w_abm_productos_old
integer x = 1531
integer y = 2008
end type

type pb_cerrar from w_abm_ancestor`pb_cerrar within w_abm_productos_old
integer x = 2080
integer y = 1844
integer taborder = 130
end type

type btn_ultimo from w_abm_ancestor`btn_ultimo within w_abm_productos_old
integer x = 1161
integer y = 1952
integer height = 100
integer taborder = 90
...
type btn_siguente from w_abm_ancestor`btn_siguente within w_abm_productos_old
integer x = 896
integer y = 1952
integer height = 100
...
type btn_anterior from w_abm_ancestor`btn_anterior within w_abm_productos_old
integer x = 631
integer y = 1952
integer height = 104
integer taborder = 50
...
type btn_primero from w_abm_ancestor`btn_primero within w_abm_productos_old
integer x = 366
integer y = 1952
integer height = 100
integer taborder = 60
...
type st_barra from w_abm_ancestor`st_barra within w_abm_productos_old
integer height = 2184
end type

type cdw_datos from w_abm_ancestor`cdw_datos within w_abm_productos_old
...
type gb_1 from w_abm_ancestor`gb_1 within w_abm_productos_old
integer x = 219
integer taborder = 30
end type

type gb_3 from w_abm_ancestor`gb_3 within w_abm_productos_old
integer x = 311
integer y = 1852
integer width = 1129
integer height = 256
...
type ln_1 from w_abm_ancestor`ln_1 within w_abm_productos_old
integer beginx = 3186
integer beginy = 1828
integer endx = 306
integer endy = 1824
...
event constructor;
setnull(g_date_null)
this.setitem( this.getrow(), 'fecha_ult_venta',  g_date_null)
end event
